    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Gerencie suas fontes de renda e negócios</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Receita Total', value: format(totalReceita), color: 'emerald' },
          { label: 'Despesa Total', value: format(totalDespesa), color: 'red' },
          { label: 'Lucro Líquido', value: format(totalLucro), color: totalLucro >= 0 ? 'emerald' : 'red' },
      ...
      {visible.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
          <p className="text-gray-500 text-sm">Nenhum negócio encontrado</p>
        </div>
      ) : (
              ...
              <div key={n.id} className={`bg-gray-900 border rounded-2xl p-5 transition-all ${n.ativo ? 'border-gray-800' : 'border-gray-800/50 opacity-60'}`}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-gray-500">Margem: {margem}%</span>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-semibold">{editing ? 'Editar Negócio' : 'Novo Negócio'}</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} className="input" placeholder="Nome do negócio" />
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1.5">Descrição (opcional)</label>
                <input value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} className="input" placeholder="Breve descrição" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm text-gray-400 mb-1.5">Receita mensal</label>
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-1.5">Despesa mensal</label>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-400">Negócio ativo</span>
              </div>
