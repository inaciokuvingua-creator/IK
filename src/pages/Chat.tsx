...
import { useWebRTC, type CallType, type PeerInfo } from '../hooks/useWebRTC';
...
export default function Chat({ initialUserId }: { initialUserId?: string }) {
  ...
  const logCall = useCallback(async (entry: { callType: CallType; status: 'answered' | 'missed' | 'declined' | 'ended'; durationSec: number }) => {
    if (!user || !activeConversationId) return;
    try {
      const statusMap: Record<string, string> = { answered: 'answered', missed: 'missed', declined: 'declined', ended: 'ended' };
      await supabase.from('chat_calls').insert({
        conversation_id: activeConversationId,
        caller_id: user.id,
        call_type: entry.callType,
        status: statusMap[entry.status] ?? 'ended',
      ...
      await supabase.from('chat_messages').insert({
        ...
        sender_id: user.id,
        type: 'call_log',
        content: `Chamada de ${entry.callType === 'video' ? 'vídeo' : 'voz'} — ${entry.status === 'answered' ? 'atendida' : entry.status === 'missed' ? 'perdida' : entry.status === 'declined' ? 'recusada' : 'terminada'}${entry.durationSec > 0 ? ` (${Math.floor(entry.durationSec / 60)}:${String(entry.durationSec % 60).padStart(2, '0')})` : ''}`,
        call_type: entry.callType,
        call_status: statusMap[entry.status] ?? 'ended',
        call_duration: entry.durationSec,
      });
    } catch (e) {
  ...
  const startCall = (type: CallType) => {
    ...
    const peer: PeerInfo = {
      ...
      avatar: activeConversation.otherAvatar,
    };
    webrtc.startCall(type, peer);
  };
  ...
  const loadConversations = useCallback(async () => {
    ...
    const summaries = await Promise.all((conversationsData ?? []).map(async (conversation: any) => {
      ...
      const lastLabel = lastMessage?.type === 'deleted' ? 'Mensagem apagada' : lastMessage?.type === 'audio' ? '🎤 Mensagem de voz' : lastMessage?.type === 'call_log' ? (lastMessage.content ?? '📞 Chamada') : lastMessage?.content ?? lastMessage?.media_name ?? 'Sem mensagens';
  ...
  const addMember = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('add_chat_member', {
        ...
        p_target_user_id: targetProfile.user_id,
      });
      if (error) throw error;
      ...
      if (data?.action === 'not_admin') {
        alert('Só o administrador pode adicionar membros.');
      }
      if (data?.action === 'not_participant') {
        alert('Não fazes parte desta conversa.');
      }
    } catch (e) {
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex flex-col">
      <div className="mb-3">
        ...
        <p className="text-gray-400 text-sm mt-0.5">Converse com compradores, fornecedores e parceiros.</p>
      </div>

      {/* Stories */}
      <div className="mb-3 bg-gray-900/50 rounded-2xl border border-gray-800 p-3">
        <Stories />
      </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-800">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-500 text-xs">{activeConversation.type === 'group' ? 'Grupo' : 'Conversa privada'}</p>
              </div>

              {/* Botões de chamada */}
              <button onClick={() => startCall('voice')} title="Chamada de voz" className="p-2 text-gray-400 hover:text-emerald-400 transition-colors shrink-0">
                <Phone size={16} />
              </button>
              <button onClick={() => startCall('video')} title="Chamada de vídeo" className="p-2 text-gray-400 hover:text-emerald-400 transition-colors shrink-0">
                <Video size={16} />
              </button>
              <button onClick={() => { setShowMembers(true); loadMembers(activeConversation.id); }} title="Membros" className="p-2 text-gray-400 hover:text-emerald-400 transition-colors shrink-0">
                <UserPlus size={16} />
              </button>
              <button onClick={clearConversation} title="Limpar conversa" className="p-2 text-gray-400 hover:text-red-400 transition-colors shrink-0">
                <Trash2 size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              {loading ? (
                ...
                const isCallLog = (message as any).type === 'call_log';
                return (
                  <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'} ${isCallLog ? 'justify-center' : ''}`}>
                    {isCallLog ? (
                      <div className="bg-gray-800/60 text-gray-300 rounded-full px-4 py-1.5 text-xs flex items-center gap-2">
                        <Phone size={12} />
                        {message.content}
                      </div>
                    ) : (
      ...
      {/* Modal de membros */}
      {showMembers && activeConversationId && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowMembers(false)}>
          <div className="bg-gray-900 rounded-2xl border border-gray-800 w-full max-w-md p-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-white font-semibold">Membros</h3>
            ...
            <div className="flex gap-2 mb-4">
              ...
              />
              <button onClick={addMember} disabled={adding || !addIdentifier.trim()} className="bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white px-3 py-2 rounded-xl text-xs flex items-center gap-1">
            ...
            {loadingMembers ? (
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto">
                {members.map((m) => (
                  <div key={m.user_id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-gray-800">
                    <div className="w-8 h-8 rounded-full bg-gray-700 overflow-hidden flex items-center justify-center text-white text-xs font-bold shrink-0">
                      {m.avatar_url ? <img src={m.avatar_url} alt="" className="w-full h-full object-cover" /> : (m.nome?.[0]?.toUpperCase() ?? 'U')}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-sm truncate">{m.nome ?? m.email ?? 'Utilizador'}</p>
      ...
      <CallModal
        ...
        seconds={webrtc.seconds}
        onAccept={webrtc.acceptCall}
        onReject={webrtc.rejectCall}
        onCancel={webrtc.cancelCall}
